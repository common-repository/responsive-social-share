 ...
 * wpspec_View
 */
if ( ! class_exists( 'wpspec_Caching' ) ) {
    
    class wpspec_Caching {
        ...
        /**
         * Check for wpspec Caching requests
         */
        public static function watcher() {
             ...
             */
            self::$cache = sanitize_key( $_POST['_wpspec_cache'], false );
            self::$lock = sanitize_key( $_POST['_wpspec_lock'], false );
            
            /**
             * Allow plugins/themes to hook into this and perform their own cache updates.
             */
            do_action( 'wpspec_caching_capture_request', self::$cache, self::$lock );
            
            /*
             * Test Refresh Script
             */
            if ( isset( $_POST['_wpspec_cache'] ) ) {
                
                ress_social_sharing_update_counts( self::$lock );
        ...
        public function spawn_process() {
            ...
            $server_url = home_url( '/?wpspec_caching_request' );
            
            $args = array( 
                'body' => array( 
                    '_wpspec_cache' => self::$cache,
                    '_wpspec_lock' => self::$lock
                ),
                'timeout' => 0.01,
                'blocking' => false,
