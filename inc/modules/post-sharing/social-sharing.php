 ...
 */
function ress_post_sharing_register_files() {

    // Register Styles
    wp_register_style( 'ress-post-sharing', RESS_POST_SHARING_URL . 'assets/css/sharelinks.css', array(), RESS_VERSION, 'all' );
    wp_register_style( 'ress-post-sharing-min', RESS_POST_SHARING_URL . 'assets/css/sharelinks.min.css', array(), RESS_VERSION, 'all' );
        
}
add_action( 'wp_enqueue_scripts', 'ress_post_sharing_register_files' );
add_action( 'admin_enqueue_scripts', 'ress_post_sharing_register_files' );

/**
 * Enqueue backend plugin scripts and styles.
 */
function ress_post_sharing_enqueue_backend( $hook_suffix ) {
    ...
    if ( 'settings_page_responsive-social-share' == $hook_suffix ) {
        ...
        if ( true == SCRIPT_DEBUG ) {
            ...
            wp_enqueue_style( 'ress-post-sharing' );
            
        } else {
            
            wp_enqueue_style( 'ress-admin-min' );
            wp_enqueue_style( 'ress-post-sharing-min' );
...
add_action( 'admin_enqueue_scripts', 'ress_post_sharing_enqueue_backend' );

/**
 * Enqueue frontend plugin scripts and styles.
 */
function ress_post_sharing_enqueue_frontend() {
 ...
 */
function ress_post_sharing_responsive_compat() {
    ...
    $max_site_width = ress_get_max_site_width();
 ...
 */
function ress_post_sharing_content_insert( $content ) {
    ...
    if ( is_singular() && is_main_query() && in_array( $post->post_type, $options['post_sharing_post_types'] ) ) {
        ...
        add_action( 'wp_footer', 'ress_post_sharing_frontend_js_print' );
        ...
        if ( in_array( 'top', $options['post_sharing_position'] ) ) {
            ...
            $content = ress_post_sharing_services_render() . $content;
        ...
        if ( in_array( 'bottom', $options['post_sharing_position'] ) ) {
            ...
            $content = $content . ress_post_sharing_services_render();
        ...
        if ( in_array( 'floating', $options['post_sharing_position'] ) ) {
            ...
            add_action( 'wp_footer', 'ress_post_sharing_floating_bar_render' );
            add_action( 'wp_footer', 'ress_post_sharing_responsive_compat' );
            
        }
        
        // Decide if we need to refresh counts
        ress_maybe_refresh_counts( $post->ID );
 ...
 */
function ress_post_sharing_floating_bar_render() {
    ...
    echo '<div class="' . implode( ' ', $classes ) . '">' . ress_post_sharing_services_render() . '</div>';
 ...
 */
function ress_get_post_sharing_services() {
    
    $services = array(
        'buffer' => array(
            'name' => 'buffer',
            'label' => __( 'Buffer', 'ress' ),
            'href' => '#'
        ),
        'delicious' => array(
            'name' => 'delicious',
            'label' => __( 'Delicious', 'ress' ),
            'href' => '#'
        ),
        'digg' => array(
            'name' => 'digg',
        ...
        'facebook' => array(
            'name' => 'facebook',
            'label' => __( 'Facebook', 'ress' ),
            'href' => '#'
        ),
        'googleplus' => array(
            'name' => 'googleplus',
            'label' => __( 'Google+', 'ress' ),
            'href' => '#'
        ),
        'linkedin' => array(
            'name' => 'linkedin',
            'label' => __( 'LinkedIn', 'ress' ),
            'href' => '#'
        ),
        'pinterest' => array(
            'name' => 'pinterest',
            'label' => __( 'Pinterest', 'ress' ),
            'href' => '#'
        ),
        'reddit' => array(
            'name' => 'reddit',
            'label' => __( 'Reddit', 'ress' ),
            'href' => '#'
        ),
        'stumbleupon' => array(
            'name' => 'stumbleupon',
            'label' => __( 'Stumbleupon', 'ress' ),
            'href' => '#'
        ),
        'tumblr' => array(
            'name' => 'tumblr',
            'label' => __( 'Tumblr', 'ress' ),
            'href' => '#'
        ),
        'twitter' => array(
            'name' => 'twitter',
            'label' => __( 'Twitter', 'ress' ),
            'href' => '#'
        ),
    );
     ...
     */
    return apply_filters( 'ress_post_sharing_services', $services );
 ...
 */
function ress_post_sharing_filter_services( $type = 'selected' ) {
    
    /*
     * Get Social Services
     */
    $all_services = ress_get_post_sharing_services();
    
    /*
     * Get the user selected services
     */
    $user_selected = get_option( 'ress_post_sharing_order' );
 ...
 * Prepare Social Sharing Links
 */
function ress_post_sharing_prepare_links() {
     ...
     */
    $services = ress_post_sharing_filter_services( 'selected' );
     ...
     */
    return ( is_admin() ) ? $services : apply_filters( 'ress_post_sharing_prepare_link', $services ) ;
 ...
 */
function ress_post_sharing_buffer_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_buffer_href' );

/*
 * Social Sharing Delicious Link Setup
 */
function ress_post_sharing_delicious_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_delicious_href' );

/*
 * Social Sharing Digg Link Setup
 */
function ress_post_sharing_digg_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_digg_href' );

/*
 * Social Sharing Facebook Link Setup
 */
function ress_post_sharing_facebook_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_facebook_href' );

/*
 * Social Sharing Google+ Link Setup
 */
function ress_post_sharing_googleplus_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_googleplus_href' );

/*
 * Social Sharing LinkedIn Link Setup
 */
function ress_post_sharing_linkedin_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_linkedin_href' );

/*
 * Social Sharing Pinterest Link Setup
 */
function ress_post_sharing_pinterest_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_pinterest_href' );

/*
 * Social Sharing Reddit Link Setup
 */
function ress_post_sharing_reddit_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_reddit_href' );

/*
 * Social Sharing Stumbleupon Link Setup
 */
function ress_post_sharing_stumbleupon_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_stumbleupon_href' );

/*
 * Social Sharing Tumblr Link Setup
 */
function ress_post_sharing_tumblr_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_tumblr_href' );

/*
 * Social Sharing Twitter Link Setup
 */
function ress_post_sharing_twitter_href( $services ) {
...
add_filter( 'ress_post_sharing_prepare_link', 'ress_post_sharing_twitter_href' );

/**
 * Renders the Social Share Buttons.
 */
function ress_post_sharing_services_render( $preview = false ) {
    ...
    $theme_view = apply_filters( 'ress_post_sharing_view_dir', 'default', $theme );
    
    /**
     * Setup an instance of the View class.
     * Allow customization using a filter.
     */
    $view = new wpspec_View(
        apply_filters(
            'ress_post_sharing_view_dir',
            RESS_PATH . 'inc/modules/post-sharing/views/' . $theme_view,
            $theme
        )
    );
    ...
    if ( $preview ) {
        ...
        $selected = ress_post_sharing_filter_services( 'preview' );
        
    } else {
        
        $selected = ress_post_sharing_prepare_links();
     ...
     * Get Share Counts
     */
    $counts = get_post_meta( $post->ID, '_ress_post_sharing_counts', true );
 ...
 */
function ress_post_sharing_standard_view_dir( $theme_view, $theme ) {
...
add_filter( 'ress_post_sharing_view_dir', 'ress_post_sharing_standard_view_dir', 10, 2 );

/*
 * Social Sharing Admin Page JS
 */
function ress_post_sharing_page_print_js() {
        ...
        jQuery(document).ready(function($) {
            ...
            $("#share-links-available, #share-links-selected").sortable({
                ...
                update: function( event, ui ) {
                    ...
                    var data = {
                        action: 'ress_save_post_sharing_order',
                        data: korder,
                        nonce: '<?php echo wp_create_nonce('ress_post_sharing_order'); ?>'
                    };
                    ...
                    $.post( ajaxurl, data, function( response ) {
                        ...
                        if ( 'true' === response.success && 'save' === response.action && window.console ) {
                            console.log( 'Responsive Social Share - Post Sharing order successfully saved.' );
 ...
 */
function ress_post_sharing_frontend_js_print() {
    ...
    echo apply_filters( 'ress_post_sharing_frontend_js', $output );
